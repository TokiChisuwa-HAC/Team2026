...
// 更新
void WataruScene::update(float delta_time) {
    // ワールドの更新
    world_.update(delta_time);
}

// 描画
void WataruScene::draw() const {
    // ワールドの描画
    world_.draw();
}

// 終了しているか？
bool WataruScene::is_end() const {
    // 終了フラグを返す
    return is_end_;
}

// 次のシーンを返す
std::string WataruScene::next() const {
    return next_scene_name_;
}

// 終了
void WataruScene::end() {
    // ワールドを消去
    world_.clear();
    // 再生中のエフェクトを停止する
    gsStopAllEffects();
